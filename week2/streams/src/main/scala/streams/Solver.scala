...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    for {
      (nextBlock, nextMove) <- b.legalNeighbors.toStream
   ...
   */
  def from(current: Stream[(Block, List[Move])],
           explored: Set[Block]): Stream[(Block, List[Move])] =
  {
    if (current.isEmpty)
      Stream.Empty
      ...
      current #::: from(candidates, explored ++ (current map (_._1)))
    }
  }
   ...
   */
  lazy val pathsFromStart: Stream[(Block, List[Move])] = from(List((startBlock, List[Move]())).toStream, Set[Block]())
